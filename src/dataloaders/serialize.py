...
def safe_to_parquet(data, path, **kwargs):
    """
    Wrapper for to_parquet that handles engine compatibility issues.
    """
    if 'engine' in kwargs and kwargs['engine'] == 'pyarrow-dataset':
        kwargs['engine'] = 'pyarrow'
    
    try:
        return data.to_parquet(path, **kwargs)
    except ValueError as e:
        if "Unsupported engine" in str(e):
            # If the error is about the engine, try with pyarrow
            kwargs['engine'] = 'pyarrow'
            return data.to_parquet(path, **kwargs)
        else:
...
class ParquetSerializer(Serializer[dd.DataFrame]):
    ...
    def save_result(
        ...
        # Make sure the engine is "pyarrow" even if it was overridden
        if "engine" in parquet_kwargs and parquet_kwargs["engine"] == "pyarrow-dataset":
    ...
    def save_result(
        ...
        path = self.get_path(f, ba)
        parquet_kwargs = {
            "engine": "pyarrow",  # Changed from "pyarrow-dataset"
            "compression": "gzip",
            "allow_truncated_timestamps": True,
            "coerce_timestamps": "us",
        }
        ...
        if self.verify_index:
            ...
            # partitions...
            _, is_monotonic = dask.compute(
                safe_to_parquet(result, path, compute=False, **parquet_kwargs),
                result.index.is_monotonic,
            )
            assert is_monotonic, "Index is not monotonic."
            ...
            safe_to_parquet(result, path, **parquet_kwargs)
