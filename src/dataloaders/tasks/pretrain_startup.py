# src/dataloaders/tasks/pretrain_startup.py
"""
This is an adapted version of pretrain.py for startup data
"""
import logging
...
from src.dataloaders.types import StartupBackground, StartupDocument, EncodedDocument
...
class MLM(Task):
    """
    Task for used with Masked language modeling for startup data.
    """
    ...
    def encode_document(self, document: StartupDocument) -> "MLMEncodedDocument":
        """Custom encode_document for startup data"""
        prefix_sentence = (
            ["[CLS]"] +
    ...
    def sop_task(self, document: StartupDocument):
        """Sequence order prediction task for startups"""
        p = np.random.rand(1)
        if p < 0.05:
...
class MLMEncodedDocument(EncodedDocument[MLM]):
    """Encoded document for masked language modeling task"""
