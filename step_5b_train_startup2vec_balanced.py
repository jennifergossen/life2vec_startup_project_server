...
def import_balanced_datamodule():
    """Import the balanced datamodule from the .py file"""
    try:
        spec = importlib.util.spec_from_file_location("step_4b_create_balanced_datamodule", "step_4b_create_balanced_datamodule.py")
        if spec is None:
            raise ImportError("Could not find step_4b_create_balanced_datamodule.py")
        ...
        print(f"❌ Error importing balanced datamodule: {e}")
        print("💡 Make sure step_4b_create_balanced_datamodule.py exists in the current directory")
        sys.exit(1)

# Import the balanced classes
StartupDataModule, StartupVocabulary = import_balanced_datamodule()
...
def main():
    parser = argparse.ArgumentParser(description="Startup Life2vec Training - Beast Mode (Balanced)")
    ...
    # After parsing args and before training, test the class distribution
    print("\n🔍 Testing class distribution in balanced datamodule...")
    ...
    datamodule.setup()
    # Count companies by survival_label in train set
    train_df = datamodule.train_dataset.data
    if 'survival_label' in train_df.columns:
        survived = train_df[train_df['survival_label'] == 1]['COMPANY_ID'].unique()
        died = train_df[train_df['survival_label'] == 0]['COMPANY_ID'].unique()
        print(f"Survived companies: {len(survived)}")
        print(f"Died companies: {len(died)}")
        print(f"Ratio survived:died = {len(survived)}:{len(died)}")
    else:
        print("survival_label column not found in train data!")
    print("\nProceeding to training...\n")

    # === Begin full main function body from original step_5_train_startup2vec.py ===
    ...
    # Setup logging
    experiment_name = f"startup2vec-balanced-{'test' if args.quick_test else 'full'}-{num_gpus}gpu-{config['hidden_size']}d"
    
    if args.use_wandb:
        try:
            logger = WandbLogger(
                project="startup-life2vec-balanced",
                name=experiment_name,
                save_dir="./logs",
                config={**hparams, **config},
                tags=["balanced", "pretraining"]
            )
