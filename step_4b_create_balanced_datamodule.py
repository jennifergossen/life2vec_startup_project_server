...
class StartupDataset(Dataset):
    ...
    def __init__(self, corpus_file: Path, vocabulary: StartupVocabulary, 
        ...
        try:
            ...
            # === DOWNSAMPLING LOGIC FOR BALANCED CLASSES (COMPANY LEVEL) ===
            # Load company-level survival labels
            ...
            if 'COMPANY_ID' in self.data.columns and 'COMPANY_ID' in company_df.columns:
                # Merge survival_label into sentence-level data
                company_df = company_df[['COMPANY_ID', 'status', 'founded_on', 'closed_on']].copy()
                # Recreate survival_label logic from survival_prediction.py
                ...
                # Merge
                self.data = self.data.merge(company_df, on='COMPANY_ID', how='inner')
                # Downsample at company level
                survived_ids = self.data[self.data['survival_label'] == 1]['COMPANY_ID'].unique()
                died_ids = self.data[self.data['survival_label'] == 0]['COMPANY_ID'].unique()
                n_survived = len(survived_ids)
                n_died = len(died_ids)
                logging.info(f"Company counts before downsampling: survived={n_survived}, died={n_died}")
                if n_survived > n_died and n_died > 0:
                    # Downsample survived companies
                    ...
                    self.data = self.data[self.data['COMPANY_ID'].isin(keep_ids)].reset_index(drop=True)
                    logging.info(f"Company counts after downsampling: survived={n_died}, died={n_died}")
                else:
                    logging.info("No downsampling applied (already balanced or not enough samples)")
            else:
                logging.warning("COMPANY_ID column not found in data or company file. No downsampling applied.")
